...
avatar.addEventListener("change", async (event) => {
    // フォルダが選択された場合、選択済みファイルリストエリアに表示
    if (0 < event.target.files.length) {
        ...
        // WebkitDirectory属性を使用してフォルダ内の全てのファイルを取得
        for (const folder of event.target.files) {
            const files = await folder.webkitGetAsEntry().createReader().readEntries();
