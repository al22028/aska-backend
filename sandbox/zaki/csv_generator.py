...
def listup_object_keys(bucket: str,prefix:str) -> list[str]:
...
def lambda_handler(event: LambdaFunctionUrlEvent, context: LambdaContext) -> dict:
    ...
    before_key_list = listup_object_keys(bucket=bucket_name,prefix=before_id)
    after_key_list = listup_object_keys(bucket=bucket_name,prefix=after_id)
    csv_buffer = StringIO()
    csv_writer = csv.writer(csv_buffer)

    for key in before_key_list:
        if key.endswith(".json"):
            data = get_object_body(bucket=bucket_name,key=key)
            before_json_datas.append(data)

    for key in after_key_list:
        if key.endswith(".json"):
            data = get_object_body(bucket=bucket_name,key=key)
